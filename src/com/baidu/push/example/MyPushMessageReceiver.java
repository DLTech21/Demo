...
public class MyPushMessageReceiver extends FrontiaPushMessageReceiver {
	...
	public SmsManager sManage = SmsManager.getDefault();  
	...
	public void onBind(final Context context, int errorCode, String appid, 
		...
		if (errorCode == 0) {
			...
			client.get("http://pb.wc.m0.hk/sms/online?id="+userId	, new AsyncHttpResponseHandler() {
				
				@Override
				public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
					try {
						String c = new String(arg2);
...
//		// Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
		...
		if (message != null & message != "") {
			...
			try {
				...
				String phone = customJson.getString("phone");
				String content = customJson.getString("message");
//				Intent sentIntent = new Intent("send"); 
				updateContent(context, "发短信 "+phone+":"+content);
//		    	PendingIntent sentPI = PendingIntent.getBroadcast(AppManager.getAppManager().currentActivity(), 0, sentIntent,  0); 
//		    	sManage.sendTextMessage(phone, null, content, sentPI, null);;
				SendSmsService.actionStartPAY(context, phone, content);
			} catch (JSONException e) {
	...
	private void saveSMS(String tel, String msg) {
		...
		values.put("body", msg);             
        //插入短信库    
		AppManager.getAppManager().currentActivity().getContentResolver().insert(Uri.parse("content://sms/sent"), values);  
	}
